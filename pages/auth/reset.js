...
export default function Reset() {
    ...
    function forgotPassword(e) {
        e.preventDefault();
        axios
            .post(`${settings.APIURL}/auth/reset-password`, {
                code: code,
                password: password,
    ...
    return (
        ...
        <img className="w-auto h-full" src="/login.png" alt="" />
            ...
            <div className={`text-center p-5 ${message == "We've sent you a secret passage." ? "" : "hidden"}`}>
                <h1 className="font-bold text-2xl">Password Changed!</h1>
            ...
            <div className={`h-full w-full xl:w-1/2 flex items-center justify-center p-10 ${message == "We've sent you a secret passage." ? "hidden" : ""}`}>
                <div>
                    ...
                    <form onSubmit={forgotPassword}>
                        <FormInputBlock label="New Password" required>
                            <input value={password} onChange={e => setPassword(e.target.value)} required className="border rounded-lg w-full p-2" type="password" />
                        </FormInputBlock>

                        <FormInputBlock label="Confirm New Password" required>
                            <input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required className="border rounded-lg w-full p-2" type="password" />
                        </FormInputBlock>
